...
const NAV_ITEMS: NavItem[] = [
	...
	{ label: 'Upload Resume', href: '/upload', type: 'route' },
];
...
export function Header() {
	...
	return (
		<header
			className={cn(
				'sticky top-0 z-50 w-full border-b backdrop-blur-lg transition-colors duration-300',
				isDark ? 'bg-zinc-950/80 border-zinc-800/60' : 'bg-white/80 border-gray-200/60',
				scrolled ? 'shadow-sm' : undefined,
			)}
		>
			<nav className="mx-auto flex h-14 w-full max-w-5xl items-center justify-between px-4">
				<div className="flex items-center gap-6">
					<Link to="/home" className="rounded-md px-2 py-1.5 font-semibold tracking-tight">
						SAGE
					</Link>
					<div className="hidden md:flex items-center gap-4 text-sm text-muted-foreground">
						{NAV_ITEMS.map((item) =>
							item.type === 'hash' ? (
								...
								<Link key={item.label} to={item.href} className="hover:text-foreground">
									{item.label}
								</Link>
							),
						)}
			...
			<MobileMenu open={open} className="flex flex-col justify-between gap-6 overflow-y-auto">
				<div className="flex flex-col gap-4">
					{NAV_ITEMS.map((item) =>
						item.type === 'hash' ? (
							<a key={item.label} href={item.href} className="text-base font-medium">
								{item.label}
							</a>
						) : (
							<Link key={item.label} to={item.href} className="text-base font-medium">
								{item.label}
							</Link>
						),
					)}
...
function MobileMenu({ open, children, className, ...props }: MobileMenuProps) {
	...
	return createPortal(
		<div
			id="mobile-menu"
			className={cn(
				'fixed top-14 right-0 bottom-0 left-0 z-40 flex flex-col overflow-hidden border-y md:hidden backdrop-blur-lg transition-colors duration-300',
				isDark ? 'bg-zinc-950/80 border-zinc-800/60' : 'bg-white/80 border-gray-200/60',
			)}
		>
			<div
